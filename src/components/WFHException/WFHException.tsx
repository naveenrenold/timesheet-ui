...
function WFHException() {
  ...
  let [approvalManagerId , updateApprovalManagerId] = useState("");
        ...
        const addException = async (exceptionDate:Moment,reason : string , approvalManagerId : string)=> {
        if(!validateException(exceptionDate, approvalManagerId)){
         return 
        }
        let requestBody = {employeeDetails['employeeId'], exceptionDate, reason, approvalManagerId}
        let response = await httpClient.post(httpClient.addException, requestBody)
        console.log(response);
     };
     const validateException = (exceptionDate:Moment, approvalManagerId : string)=> {
       if(!exceptionDate){
  ...
  return (
    ...
    <div className="common"> 
      ...
      <LocalizationProvider dateAdapter={AdapterMoment}>
        <DatePicker maxDate = {moment(new Date())} onChange={(e)=>{updateExceptionDate(e!)}}
        />
    ...
    <div className="common">
      <label>Reason :</label> 
      <TextField className="MuiTextField-root" onChange={(e)=>{updateReason(e.target.value)}}/>
    </div>
    
    <div className="common">
      <label>Approval Manager Id :</label>
      <TextField className="MuiTextField-root" onChange={(e)=>{updateApprovalManagerId(e.target.value)}}/>
    </div>
    
    <div className="common">
      <button className="button" onClick={(e)=>{addException(exceptionDate,reason,approvalManagerId)}}>Submit</button>
    </div>
  </div>
    </>
  );
